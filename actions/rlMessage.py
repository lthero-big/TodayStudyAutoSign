...
class RlMessage:
    # 初始化类
    def __init__(self, sendKey, apiUrl, msgKey, sendType,username=''):
        now_time = int(str(Utils.getAsiaTime()).split(':')[0])
        if (12 > now_time >= 6):
            self.time_message = '上午好'
        elif (17 > now_time >= 12):
            self.time_message = '下午好'
        else:
            self.time_message = '晚上好'
    ...
    def sendMail(self, status, msg):
        ...
        params = {
            ...
            'content': f'[<p>{Utils.getAsiaDate()} {Utils.getAsiaTime()}]</p>{msg}'
        }
